...
public class EventManager {
    ...
    */
    ...
    public List<Event> getFilteredEvents(EventFilter filter) {
        ...
        // Jos kyseessä on DateFilter tai DateCategoryFilter, haetaan tapahtumat päivämäärän mukaan
        if (filter instanceof DateFilter) {
            MonthDay date = ((DateFilter) filter).getMonthDay();
            for (EventProvider provider : eventProviders) {
                allEvents.addAll(provider.getEventsOfDate(date));
            }
        } else {
            // Muissa tapauksissa käytetään getEvents()-metodia
            for (EventProvider provider : eventProviders) {
                allEvents.addAll(provider.getEvents());
            }
        }

        // Suodatetaan tapahtumat annetun filtterin mukaan
